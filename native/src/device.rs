...
fn enumerate_hid_devices() -> Result<Vec<Device>> {
    ...
    for device_info in api.device_list() {
        ...
        let manufacturer = device_info.manufacturer_string()
            .map(|s| s.to_string());
        let product_name = device_info.product_string()
            .map(|s| s.to_string());
        ...
        log::info!(
            ...
            device.product_name.as_ref().unwrap_or(&"Unknown".to_string()),
            device.vendor_id,
            device.product_id,
            device.path
        );
...
fn enumerate_ccid_devices() -> Result<Vec<Device>> {
    ...
    let ctx = match pcsc::Context::establish(pcsc::Scope::User) {
        Ok(ctx) => ctx,
        Err(e) => {
            log::warn!("Failed to establish PC/SC context: {}. Skipping CCID enumeration.", e);
            return Ok(Vec::new());
        }
    };
    ...
    let readers_buf = match ctx.list_readers_owned() {
        Ok(buf) => buf,
        Err(e) => {
            log::warn!("Failed to list PC/SC readers: {}. No CCID devices found.", e);
            return Ok(Vec::new());
        }
    };
    ...
    for reader_name in readers_buf.iter() {
        ...
        let (manufacturer, product_name, serial_number) = match ctx.connect(reader_name, pcsc::ShareMode::Shared, pcsc::Protocols::ANY) {
            Ok(card) => {
